.PHONY: help start start-force start-chap start-chap-force start-chapkit start-chapkit-force clean ps logs route
...
CHAPKIT_FILE := compose.chapkit.yml
# compose.chap.yml `include`s compose.yml (DHIS2 + chap-core, no model services).
# compose.chapkit.yml `include`s compose.chap.yml plus the per-model overlays
# (e.g. compose.ewars.yml) — the umbrella that adds the bundled chapkit models.
# Both drive the same compose project, so the manage targets use the umbrella
# (a superset) to also tear down / inspect any running model services.
...
help:
	...
	@echo "  start-chapkit       Start DHIS2 + chap-core + chapkit models e.g. EWARS (compose.chapkit.yml)"
	@echo "  start-chapkit-force Recreate DHIS2 + chap-core + chapkit models from scratch (wipes all volumes)"
	@echo ""
...
# --- start: DHIS2 + chap-core + chapkit models --- (foreground; Ctrl+C to stop)
start-chapkit:
	@echo ">>> Starting DHIS2 + chap-core + chapkit models (compose.chapkit.yml) — Ctrl+C to stop"
	@$(COMPOSE) -f $(CHAPKIT_FILE) up --remove-orphans

start-chapkit-force:
	@echo ">>> Recreating DHIS2 + chap-core + chapkit models from scratch (removing volumes) — Ctrl+C to stop"
	@$(COMPOSE) -f $(CHAPKIT_FILE) down -v --remove-orphans
	@$(COMPOSE) -f $(CHAPKIT_FILE) up --remove-orphans

# --- manage (operate on the whole project, chap + model services included) ---
clean:
	@echo ">>> Stopping and removing containers and volumes"
	@$(COMPOSE) -f $(CHAPKIT_FILE) down -v

ps:
	@$(COMPOSE) -f $(CHAPKIT_FILE) ps -a

logs:
	@$(COMPOSE) -f $(CHAPKIT_FILE) logs -f
